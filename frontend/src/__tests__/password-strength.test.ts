...
describe("getPasswordStrength", () => {
	...
	it("rewards length — a long lowercase phrase beats a short symbol-heavy one", () => {
		const longLower = getPasswordStrength("correct horse battery staple");
		const shortSymbol = getPasswordStrength("P@s$!");
		expect(longLower.score).toBeGreaterThan(shortSymbol.score);
	});
});
