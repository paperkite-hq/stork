...
- Stork **never modifies your IMAP server**. The sync is read-only — no messages are deleted, moved, or modified on the server. It's safe to use against a production mailbox.
