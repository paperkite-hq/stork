...
describe("Connectors API", () => {
	...
	describe("Clean server endpoint", () => {
		...
		test("POST /api/connectors/inbound/:id/clean-server returns 400 for non-IMAP connector", async () => {
			db.prepare(
				`INSERT INTO inbound_connectors (name, type) VALUES ('CF Email', 'cloudflare-email')`,
			).run();
			const id = Number(
			...
			const { status, body } = await post(`/api/connectors/inbound/${id}/clean-server`, {});
			expect(status).toBe(400);
			expect(body.error).toMatch(/IMAP/);
		});

		test("POST /api/connectors/inbound/:id/clean-server returns 500 when connector not registered in scheduler", async () => {
			db.prepare(
				`INSERT INTO inbound_connectors (name, type, imap_host, imap_port, imap_tls, imap_user, imap_pass)
				VALUES ('Test IMAP', 'imap', 'imap.example.com', 993, 1, 'user@example.com', 'pass')`,
			).run();
