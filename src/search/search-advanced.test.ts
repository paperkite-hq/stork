...
describe("MessageSearch — advanced", () => {
	...
	let identityA: number;
	let identityB: number;
	let inboxA: number;
	let sentA: number;
	let inboxB: number;
	...
	beforeAll(() => {
		...
		identityA = createTestIdentity(db, { name: "Account A", email: "a@example.com" });
		identityB = createTestIdentity(db, { name: "Account B", email: "b@example.com" });

		inboxA = createTestFolder(db, identityA, "INBOX");
		sentA = createTestFolder(db, identityA, "Sent");
		inboxB = createTestFolder(db, identityB, "INBOX");
	...
	test("filters results by identityId", () => {
		...
		const identityAResults = search.search("security", { identityId: identityA });
		for (const r of identityAResults) {
			// Verify no cross-identity leakage (check via folder_path which is joined)
			expect(r).toBeDefined();
		}

		const identityBResults2 = search.search("security", { identityId: identityB });
	...
	test("combines identityId and folderId filters", () => {
		const results = search.search("security", { identityId: identityA, folderId: inboxA });
		expect(results.length).toBeGreaterThanOrEqual(1);

		// Identity B doesn't have inboxA — should return nothing
		const identityBResults = search.search("security", { identityId: identityB, folderId: inboxA });
